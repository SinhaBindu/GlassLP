...
namespace GlassLP.Controllers
{
	...
	public class UsersController : BaseController
	{
		...
		public UsersController(
			...
			CommonData commonData, SPManager sPManager, GlobalDataService globalData)
		{
		...
		public async Task<IActionResult> Create(string? id)
		{
			...
			if (!string.IsNullOrEmpty(id))
			{
				...
				if (user != null)
				{
					...
					//
					var datamap= _context.AspUserMap.Where(x => x.UserId == user.Id && x.DistrictId != null).Select(x=>new
					{
						DistrictId = x.DistrictId,
						BlockId= x.BlockId,
						CLFId= x.CLFId,	
					}).ToList();
					var selectedDistrictIds = datamap.Select(x => x.DistrictId).ToList();
					if (selectedDistrictIds.Any())
					{
						model.DistrictList = await _context.MstDistrict
									.Where(x => x.IsActive == true)
									.Select(x => new SelectListItem
									{
										Value = x.DistrictId_pk.ToString(),
										Text = x.DistrictName,
										Selected = selectedDistrictIds.Contains(x.DistrictId_pk)
									})
									.ToListAsync();
					}
					// Get user role
		...
		public async Task<Result<ApplicationUser>> Create(UserViewModel model)
		{
			...
			try
			{
				...
				bool isNewUser = string.IsNullOrWhiteSpace(model.Id);
				if (isNewUser)
				{
					...
					if (createResult.Succeeded)
					{
						...
						{
							if (!string.IsNullOrWhiteSpace(user.Id) && !string.IsNullOrWhiteSpace(model.RoleId))
							{
								//var ids = model.DistrictIds.Count > 0 ?
								//.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(x => int.Parse(x.Trim())).ToList();
								foreach (var id in model.DistrictIds.ToList())
								{
									var objs = new AspUserMap
									{
										UserId = user.Id,
										RoleId = model.RoleId,
										DistrictId = id,
										BlockId = model.BlockId ?? null,
										CLFId = model.CLFId ?? null,
									};
									_context.AspUserMap.Add(objs);
								}
							}
							ressuccee = await _context.SaveChangesAsync();
							if (ressuccee > 0)
								ressuccee = 1;
						}
					}
				...
				else
				{
					...
					{
						ressuccee = 1;
						var tblmapdata = _context.AspUserMap.Where(x => x.UserId == model.Id && x.DistrictId != null).ToList();
						if (tblmapdata.Any())
						{
							_context.AspUserMap.RemoveRange(tblmapdata);
				...
				var message = isNewUser && ressuccee == 1 ? "User created successfully!" : (isNewUser && ressuccee == 2 ? "User updated successfully!" : "Record not update"); ;
		...
		public async Task<Result<string>> GetUserList()
		{
			try
			{
				DataTable dt = _sPManager.SP_UserList();
				if (dt.Rows.Count > 0)
				{
					string html = RenderPartialViewToString("_UserData", dt);
